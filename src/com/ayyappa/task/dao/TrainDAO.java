...
import com.ayyappa.task.utils.DBUtil;
...
public class TrainDAO {
	...
	public boolean save(TrainDTO dtos) {
		...
		try {
			...
			stat.setObject(4, dtos.getDestinaation());
	...
	public Collection<TrainDTO> getAll() {
		...
		try {
			...
			while (result.next()) {
				...
				dtos.setDestinaation(Destination);
				dtos.setTrain_seat_No(Train_seat_No);
				dtos.setPrice(Train_Price);
				...
				System.out.println("train_id: " + Train_id + " " + "train_no: " + Train_no + " " + "boarding: "
						...
						+ "  " + "price: " + Train_Price + "  " + "train_class: " + coach_class + "  " + "name: "
						+ name);
	...
	public boolean updateDestinationByTrainNo(String newDest, String trainNo) {
		...
		try {
			...
			String sql = "Update train " + "set Train_destinaation=?" + "where train_no=? ";
			PreparedStatement stat = mysql.prepareStatement(sql);
	...
	public int getMinPrice() {
		...
		try {
			...
			String sql = "select min(train_price) from train";
			PreparedStatement stat = mysql.prepareStatement(sql);
			ResultSet result = stat.executeQuery();
			while (result.next()) {
